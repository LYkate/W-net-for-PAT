...
def train_net(net, device, train_input_root,train_lable_root, batch_size=20, lr=0.0001):
    # 加载训练集
    ...
    optimizer = torch.optim.Adam(net.parameters(), lr, weight_decay=1e-4)#权重衰减
    # 创建一个学习率调度器
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[30, 70], gamma=0.1, last_epoch=-1)
    # 定义Loss算法
